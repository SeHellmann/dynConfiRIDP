...
#' paramDf <- data.frame(
#'   a = 2.5, v1 = 0.1, v2 = 1, t0 = 0.1, z = 0.55,
...
#' ggplot(simus, aes(
#'   x = rt, group = interaction(correct, rating),
...
simulate_dynaViTE_formula <- function(beta, pred_matrix = NULL, fixed = NULL, # method="simulation_matrix",
  ...
  for (i in 1:length(man_pars)) {
    manipulations[[man_pars[[i]][1]]] <- c(
      manipulations[[man_pars[[i]][1]]],
      man_pars[[i]][2]
    )
  }
  ...
  res <- r_WEV_matrix(parammatrix[, c(1:4, 6:8, 5, 9, 12:16)],
    delta = delta,
