fittingdynWEV_formula <- function(DVs, model_matrix,
  ...
  #### Create grid ####
  if (!simult_conf && !(is.numeric(restr_tau) && restr_tau > 0)) {
    stop(paste("restr_tau must be numeric and positive, Inf or 'simult_conf'. But restr_tau=", restr_tau, sep = ""))
  }
  ...
  if (grid_search) {
    ...
    if (useparallel) {
      logL <-
        parApply(cl, inits,
          MARGIN = 1, # loops/parallelizes over first dim (rows)
          function(p) {
            try(
              neglikelihood_formula(
                p, DVs,
                model_matrix, fit_pars_columns,
...
neglikelihood_formula <- function(beta, DVs,
  ...
  for (i in 1:length(parnames)) {
    if (parnames[i] %in% names(fixed)) {
      ...
      if (parnames[i] %in% names(fit_pars_columns)) {
  ...
  parammatrix[, c("th1", "th2")] <- cbind(
    Thetas[(DVs$response) * (nRatings + 1) + DVs$rating],
