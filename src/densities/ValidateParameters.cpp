...
void logError(const std::string& paramName, double value, const std::string& message) {
    Rcpp::Rcout << "[param validation error]: parameter '" << paramName << "' with value " << value << " " << message << std::endl;
}
...
bool ValidateParams(Rcpp::NumericVector params) {
    ...
    if (params[2] - fabs(0.5 * params[3]) - 0.5 * params[6] < 0) {
        valid = false;
        Rcpp::Rcout << "[param validation error]: invalid parameter combination t0 = " 
    ...
    if (params.size() <= 10) { // DDMConf model
        ...
        if (params[9] < params[8]) {
            valid = false;
            Rcpp::Rcout << "[param validation error]: invalid parameter combination th1 = " 
                << params[8] << ", th2 = " << params[9] 
                << ", th2 must be < th1" << std::endl;
        }
