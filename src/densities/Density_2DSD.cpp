/* Density_2DSD.cpp - Functions for PDF calculation of the 2DSD model
 *
...
double g_minus_2DSD(double t, Rcpp::NumericVector params);

double integral_t0_g_minus_2DSD(double t, Rcpp::NumericVector params);
double integral_z_g_minus_2DSD(double t, Rcpp::NumericVector params);
double integral_v_g_minus_2DSD(double t, double zr, Rcpp::NumericVector params);

double g_minus_no_var_2DSD(double t, double a, double zr, double v, double tau,
                           double th1, double th2, double lambda);
double g_minus_small_time_2DSD(double t, double zr, int N);
double g_minus_large_time_2DSD(double t, double zr, int N);

// TODO: Make sure these function names are accurate
double integrate_z_over_t_2DSD(Rcpp::NumericVector params, double a, double b,
                               double step_width);
double integrate_v_over_zr_2DSD(Rcpp::NumericVector params, double a, double b,
...
double integral_t0_g_minus_2DSD(double t, Rcpp::NumericVector params) {
  ...
  if (params[6] < params[15]) // 170501   was == 0)
  {
    ...
    res = integrate_z_over_t_2DSD(params, t - .5 * params[6],
                                  t + .5 * params[6], params[12]) /
          params[6];
  }
...
double integral_z_g_minus_2DSD(double t, Rcpp::NumericVector params) {
  ...
  if (params[4] < params[14]) {
    ...
    res = integrate_v_over_zr_2DSD(params, params[7] - .5 * params[4],
                                   params[7] + .5 * params[4], t, params[13]) /
          params[4];
  }
  return res;
}

double integral_v_g_minus_2DSD(double t, double zr,
  ...
  sv2t = sv * sv * t + 1;
  factor = 1 / (a * a * sqrt(sv2t)) *
           exp(-0.5 * (v * v * t + 2 * a * zr * v - a * a * zr * zr * sv * sv) /
               (sv2t)); // sqrt(t+tau)

  mean_conf = -(tau * v - a * zr * (sv * sv * (tau + t) + 1)) /
              (sv2t); // ((v - sv*sv*a*zr)*tau) / sv2t;
  sd_conf = sqrt(tau * (sv * sv * tau + sv2t) / (sv2t));
  diff_normal =
      0.5 *
      (erf((th2 * pow(t + tau, lambda) - mean_conf) / (M_SQRT2 * sd_conf)) -
       erf((th1 * pow(t + tau, lambda) - mean_conf) / (M_SQRT2 * sd_conf)));

  // factor = exp(-a*zr*v - 0.5*v*v*t) / (a*a); //sqrt(t+tau)
  // diff_normal = 0.5*(erf((th2*pow(t+tau, lambda) + (tau*v - a*zr)) / (M_SQRT2
  // * sqrt(tau))) -
  ...
  if (M_PI * ta * eps < 1) {
    N_large = std::max(N_large, (int)ceil(sqrt(-2 * log(M_PI * ta * eps) /
                                               (M_PI * M_PI * ta))));
  }

  if (2 * sqrt(2 * M_PI * ta) * eps < 1) {
    N_small = (int)ceil(fmax(
        sqrt(ta) + 1, 2 + sqrt(-2 * ta * log(2 * eps * sqrt(2 * M_PI * ta)))));
  } else {
    N_small = 2;
  }
...
double g_minus_no_var_2DSD(double t, double a, double zr, double v, double tau,
  ...
  factor = exp(-a * zr * v - 0.5 * v * v * t) / (a * a); // sqrt(t+tau)
  diff_normal = 0.5 * (erf((th2 * pow(t + tau, lambda) + (tau * v - a * zr)) /
                           (M_SQRT2 * sqrt(tau))) -
                       erf((th1 * pow(t + tau, lambda) + (tau * v - a * zr)) /
                           (M_SQRT2 * sqrt(tau))));
...
// CONVERSION NOTE: Simplest way to deal with the integrate function is to
// remove
...
Rcpp::NumericVector density_2DSD(Rcpp::NumericVector rts,
